module "rocky9-vault-test" {
  source                 = "github.com/stuttgart-things/vsphere-vm?ref=v1.7.5-2.7.0-1"
  vm_count               = 1
  vsphere_vm_name        = "rocky9-vault-test"
  vm_memory              = 4096
  vsphere_vm_template    = "sthings-r9"
  vm_disk_size           = "32"
  vm_num_cpus            = 2
  firmware               = "bios"
  ...
  annotation             = "VSPHERE-VM rocky9-vault-test sthings-r9 BUILD w/ TERRAFORM FOR STUTTGART-THINGS"
...
output "ip" {
  value = [module.rocky9-vault-test.ip]
}
