...
source "vsphere-iso" "autogenerated_1" {
  ...
  boot_command         = ["<up><wait5><tab> inst.text inst.repo=cdrom:sr0 inst.ks=cdrom:sr1:/ks.cfg<enter><wait>"]
  boot_order           = "disk,cdrom"
  boot_wait            = "{{ .bootWait }}"
  cd_files             = ["{{ .packerConfigMountPath }}/ks.cfg"]
  cd_label             = "cidata"
  cluster              = "{{ .cluster }}"
  ...
  vm_name        = "{{ .osVersion }}-{{ .ansibleOsProvioning }}"
}
