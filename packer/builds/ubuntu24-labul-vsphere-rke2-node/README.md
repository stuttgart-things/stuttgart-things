...
this file was created at: 2024-05-05 09:40
...
```bash
# CLONE FROM BRANCH
git clone --branch ubuntu24-labul-vsphere https://github.com/stuttgart-things/stuttgart-things

# OR CLONE FROM MAIN AFTER A PREVIOUS PR-MERGE
git clone https://github.com/stuttgart-things/stuttgart-things

# CHANGE TO DIR
cd stuttgart-things/packer/builds/ubuntu24-labul-vsphere/
```
...
packer init packer/builds/ubuntu24-labul-vsphere-rke2-node/ubuntu24-rke2-node.pkr.hcl

DATE=$(echo $(date +'%Y-%m-%d'))

packer build -force \
-var "name=${DATE}-ubuntu24-labul-vsphere" \
-var "password=<PASSWORD>" \
-var "password=<PASSWORD>" \
packer/builds/ubuntu24-labul-vsphere-rke2-node/ubuntu24-rke2-node.pkr.hcl
```
...
cd ubuntu24-labul-vsphere-rke2-node/test-vm/
